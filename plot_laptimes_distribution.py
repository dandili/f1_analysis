...
race = fastf1.get_session(2022, "Dutch", 'R')
race.load()
...
# Lists to store the x and y values for the line plot
x_vals = []
y_vals = []


# Iterate through the finishing_order
for driver in finishing_order:
    # Find the minimum lap time for this driver
    min_time = driver_laps[driver_laps['Driver'] == driver]['LapTime(s)'].min()
    x_vals.append(driver)
    y_vals.append(min_time)

# Plot a line connecting each driver's fastest lap time
ax.plot(x_vals, y_vals, color='white', linestyle='-', marker='o', markersize=4, alpha=0.6)

###############################################################################
# Make the plot more aesthetic
ax.set_xlabel("Driver")
ax.set_ylabel("Lap Time (s)")
plt.suptitle("2023 British Grand Prix Lap Time Distributions")
